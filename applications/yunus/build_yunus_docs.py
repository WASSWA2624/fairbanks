#!/usr/bin/env python3
"""Build the Yunus Centre social business partnership pack for FairBanks.

Creates one synchronized set:
  documents/yunus_word.docx
  documents/yunus_pdf.pdf
  documents/yunus_ppt.pptx

Send-ready email lives in partnership_email.md (not in Word/PDF).
Narrative: social business partnership with Yunus Centre. Three pillars + FCHIP. Human tone.
"""
...
ASSETS = REPO / "assets"
OUT = HERE / "documents"
DOCX = OUT / "yunus_word.docx"
PDF = OUT / "yunus_pdf.pdf"
PPTX = OUT / "yunus_ppt.pptx"
TMP = REPO / "tmp" / "yunus"
...
SUBTITLE = "Prepared for the Yunus Centre and social business network"
...
COVER_HEADLINE = "Community health as a social business"
COVER_SUMMARY = (
    ...
    "home. This brief invites a conversation with the Yunus Centre about social business "
    "approaches to community health, financial protection, and livelihoods."
)
...
# ---- Cover ----
    ...
    add_para("Why a social business partnership matters", bold=True, size=12, color=ORANGE, space_after=6)
    ...
    value_cols = [
        (
            "For Yunus Centre",
            [
                "A living social business case in community health from Uganda",
                "A partner advancing health access through sustainable enterprise methods",
                "A field model linking care, CHIS, livelihoods, and learning",
            ],
            PALE_ORANGE,
        ),
        (
            "For FairBanks",
            [
                "Learning and visibility within the social business movement",
                "Advice and introductions across the Yunus / Grameen social business network",
                "Guidance as we develop the FCHIP MVP as a social business tool",
            ],
            PALE_TEAL,
        ),
        (
    ...
    add_para(
        "FairBanks is asking to explore a partnership with the Yunus Centre. "
        ...
        "platform. We are looking for collaborators who care about social business in practice - "
        "sustainable models that expand access to care and strengthen families.",
        size=11,
        color=SLATE,
        space_after=10,
    )
    ...
    add_bullets(
        [
            ...
            "A partnership focused on social business: field delivery, prevention, and learning",
        ]
    )
    ...
    summary_rows = [
        ("Area", "Detail"),
        ("Yunus Centre / social business network", "Global hub for social business learning, networks, and practice"),
        ("FairBanks", "Operating clinic, Community Reach work, FCHIP MVP in development, and local relationships"),
        ("Shared aim", "Stronger community health outcomes through social business methods"),
    ]
    ...
    add_para(
        "Yunus Centre promotes social business: solving social problems through sustainable "
        "business methods, including healthcare and services for people with limited means. "
        "FairBanks works in that spirit through clinics, Community Reach, CHIS, livelihoods, "
        "and tools that help people get care earlier and closer to home.",
    )
    add_para("Areas we would like to explore", style="Heading 2", bold=True, size=13, color=TEAL)
    add_bullets(
        [
            "Community health as a social business in Uganda and East Africa",
            "Preventive healthcare and health promotion close to home",
            "CHIS and livelihood pathways as financial protection",
            "Digital health / FCHIP for earlier action in underserved communities",
            "Learning from Grameen and Yunus Centre health and social business practice",
            "Exchange with the Yunus Social Business Centre (YSBC) network",
            "Advice and introductions while we develop the FCHIP MVP",
            "Social business growth and mentorship",
        ]
    )
    ...
    add_para(
        "We would welcome a conversation about how the Yunus Centre / social business network and FairBanks can "
        "work together on community health as a social business. As we develop the FCHIP MVP, we are looking "
        "for partners who can help us learn, offer advice, and open useful introductions in the network.",
    )
...
def convert_pdf() -> None:
    ...
    tmp_pdf = TMP / "yunus_pdf_build.pdf"
    if tmp_pdf.exists():
        tmp_pdf.unlink()
...
def build_pptx() -> None:
    ...
    band(
        s,
        "Project summary",
        "Why FairBanks fits the social business idea",
        "Health, livelihoods, and sustainable community enterprise.",
    )
    cards = [
        (
            "For Yunus Centre",
            ...
            PALE_ORANGE,
            [
                "A live clinic and community enterprise",
                "Social goals delivered through real care",
                "A partner grounded in daily community health work",
            ],
        ),
        (
            "For FairBanks",
            ...
            PALE_TEAL,
            [
                "Learning exchange and network visibility",
                "Advice and useful network introductions",
                "Guidance as we develop the FCHIP MVP as a social business tool",
            ],
        ),
        (
    ...
    band(
        s,
        "Why Yunus Centre",
        "From social business principles to community action",
        "Health access, livelihoods, and stronger communities",
    )
    photo_pair(
        s,
        "doctor_hands",
    ...
    bullets(
        s,
        [
            "Yunus Centre champions social business to solve human problems",
            "FairBanks turns social goals into access and prevention",
            "FCHIP helps communities see risk earlier",
            "Global social business learning + local health delivery",
    ...
    bullets(
        s,
        [
            "Community health as a social business",
            "CHIS and livelihood pathways",
            "FCHIP / digital health for earlier action",
            "Learning exchange with Yunus Centre & YSBC",
            "Advice while we develop the FCHIP MVP",
        ],
        0.8,
        2.25,
        5.3,
    ...
    text(
        s,
        "We would welcome a conversation about community health as a social business.",
        0.8,
        1.7,
        10.5,
        1.5,
...
def validate() -> None:
    ...
    for phrase in ("Subject:", "FCHIP", CONTACT_NAME, "yunus_pdf.pdf"):
...
def main() -> None:
    print(f"Building Yunus Centre partnership pack ({date.today().isoformat()})")
    OUT.mkdir(parents=True, exist_ok=True)
    build_docx()
    convert_pdf()
    build_pptx()
