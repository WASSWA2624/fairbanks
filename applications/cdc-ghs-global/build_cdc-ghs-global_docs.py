#!/usr/bin/env python3
"""Build CDC-RFA-JG-26-0056 FairBanks application pack (global GHS detect/notify/respond).

Creates one synchronized set:
  documents/cdc-ghs-global_word.docx
  documents/cdc-ghs-global_pdf.pdf
  documents/cdc-ghs-global_ppt.pptx

Curated copy-paste answers live in application_answers.md (not overwritten).
Run: python applications/cdc-ghs-global/build_cdc-ghs-global_docs.py
"""
...
PROGRAMME = (
    "CDC-RFA-JG-26-0056 - Strengthening Global Health Security by improving "
    "public health capacity to detect, notify, and respond to disease outbreaks globally"
)
...
CRITICAL = [
    ["Opportunity number", "CDC-RFA-JG-26-0056"],
    ["Applicant", ORG_LEGAL],
    ["Company No. / TIN / NSSF", f"{COMPANY_NO} · {TIN} · {NSSF}"],
    ["Countries Year 1", "Uganda (primary); East Africa corridor learning (CONFIRM LOIs)"],
...
SHARED_WIN = [
    ...
    ["Uganda / MoH", "Last-mile signals into NISS pathways; trained CHWs; 7-1-7 practice"],
...
ABSTRACT = (
    ...
    f"Slogan: {SLOGAN} "
    "This NOFO strengthens countries' capacity to independently detect, notify, and respond "
    "to disease threats of international significance. Uganda's last-mile gap remains clear: "
    "community and facility signals that reach decision-makers too late; limited surge-ready "
    "workforce at subnational level; and incomplete links into national surveillance such as "
    "NISS pathways. Under Ministry of Health leadership, Year 1 Component 1 will (1) strengthen "
    "community and facility surveillance that contributes to MoH/NISS channels using FairBanks "
    "FCHIP tools, (2) speed community-to-district detection and response using 7-1-7 style "
    "timing, (3) train CHWs/VHTs and frontline workers for One Health-aware surveillance and "
    "surge readiness, and (4) tighten community-facility links for priority diseases. "
    "Laboratory and surge modules will be delivered with MoH-aligned partners. Components 2-5 "
    "are contingency surge plans that may be approved but unfunded until CDC activates "
    "emergency funding. Year 1 country of performance: Uganda (primary), with East Africa "
    "corridor learning under MoH Uganda. Shared result: threats found and acted on closer to "
    "source - safer for Uganda and for the United States."
)

PROBLEM = [
    "Uganda faces repeated infectious disease threats shaped by geography, rapid urban growth, "
    "and high population movement across East Africa corridors. Outbreaks that start in "
    "peri-urban communities can grow before national systems see them. Americans and Ugandans "
    "are safer when threats are found and contained close to source.",
...
APPROACH = [
    ...
    "available to MoH and CDC for appropriate use. Multi-country claims beyond Uganda require "
    "registration or partner LOIs (CONFIRM before naming additional countries).",
]

STRATEGIES = [
    ["1 Surveillance systems with governments", "FCHIP capture -> MoH/NISS-aligned exports; data quality; simple GIS early warning"],
    ["2 Interoperable public health data", "Community-facility-lab signal integration; weekly reports; district dashboards"],
    ["3 Public health programmes / service links", "Community-facility links for HIV, TB, malaria, VHFs, mpox, immunisation"],
    ["4 Assessment and corrective action", "Joint district reviews; AARs after drills; continuous improvement"],
    ["5 Workforce", "CHW/VHT and facility training; surge roster; One Health-aware sessions"],
    ["6 Networks and surge connect", "Partner coordination; MoH/CDC sharing; surge activation pathways"],
    ["7 Outbreak response (C2-C5)", "Contingency playbooks for moderate/substantial PHEs when CDC activates funds"],
]
...
CAPACITY = [
    ...
    "Year 1 stays Uganda-primary; multi-country growth via registered partners.",
]
...
def build_pptx() -> None:
    ...
    textbox(sl, Inches(6.2), Inches(5.6), Inches(6.5), Inches(0.6), f"CDC-RFA-JG-26-0056 · Due {DEADLINE}", size=12, color="B8C4C8")
    add_fade(sl)
    ...
    bullets(
        ...
        [
            ...
            "Create a workspace for CDC-RFA-JG-26-0056; AOR must Sign and Submit.",
